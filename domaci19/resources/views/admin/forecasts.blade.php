...
{{--            @dd($cities->items())--}}
            ...
            @foreach($city->forecasts as $forecast)
                    <li>
                        {{$forecast->date}} -
                        <i class="{{Functions::whatIsWeatherType($forecast->weather_type)}}"></i>
                        <span class="{{Functions::addTmpColor($forecast->temperature)}}">
                            {{$forecast->temperature}}°C
                        </span>
                        | Šanse za padavine: {{$forecast->probabbility ?? '0'}}%
